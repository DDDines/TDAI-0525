...
class StatusEnriquecimentoEnum(str, enum.Enum):
    """Encapsulates Status enriquecimento enum."""
    ...
    NAO_APLICAVEL = (
        "NAO_APLICAVEL"  # Para produtos onde enriquecimento web não se aplica
    )


class StatusGeracaoIAEnum(str, enum.Enum):
    """Encapsulates Status geracao i a enum."""
    NAO_INICIADO = "NAO_INICIADO"
    PENDENTE = "PENDENTE"
    EM_PROGRESSO = "EM_PROGRESSO"
    CONCLUIDO = "CONCLUIDO"
...
class TipoAcaoEnum(str, enum.Enum):
    ...
    ENRIQUECIMENTO_WEB_PRODUTO = (
        "enriquecimento_web_produto"  # Ação de buscar dados na web com ou sem IA
    )
...
class TipoAcaoSistemaEnum(str, enum.Enum):
    """Encapsulates Tipo acao sistema enum."""
    CRIACAO = "CRIACAO"
...
class AttributeFieldTypeEnum(str, enum.Enum):
    """Encapsulates Attribute field type enum."""
    TEXT = "text"
    NUMBER = "number"
    BOOLEAN = "boolean"
    SELECT = "select"  # Dropdown com uma única seleção
    MULTISELECT = (
        "multiselect"  # Lista de checkboxes ou similar para múltiplas seleções
    )
    DATE = "date"
...
class User(Base):
    ...
    )  # Assumindo que o default role será "user"
    ...
    produtos = relationship(
        "Produto", back_populates="owner", cascade="all, delete-orphan"
    )
    fornecedores = relationship(
        "Fornecedor", back_populates="owner", cascade="all, delete-orphan"
    )
    product_types_criados = relationship(
        "ProductType", back_populates="owner", cascade="all, delete-orphan"
...
class Plano(Base):
    ...
    permite_api_externa = Column(
        Boolean, default=False
    )  # Se o plano permite acesso via API
...
class ProductType(Base):
    ...
    key_name = Column(
        String,
        nullable=False,
        comment="Chave única para identificar o tipo, ex: 'smartphones'",
    )  # Não necessariamente unique globalmente se user_id estiver presente
    friendly_name = Column(
        String, nullable=False, comment="Nome amigável, ex: 'Smartphones'"
    )
    description = Column(Text, nullable=True)
    ...
    user_id = Column(
        ...
        ForeignKey("users.id"),
        nullable=True,
        comment="Se NULL, é um tipo global/padrão. Senão, pertence a um usuário.",
    )
    ...
    attribute_templates = relationship(
        "AttributeTemplate", back_populates="product_type", cascade="all, delete-orphan"
    )
...
class AttributeTemplate(Base):
    """Encapsulates Attribute template."""
    ...
    attribute_key = Column(
        String, nullable=False, comment="Chave do atributo, ex: 'cor', 'memoria_ram'"
    )
    label = Column(
        String,
        nullable=False,
        comment="Nome de exibição do atributo, ex: 'Cor', 'Memória RAM'",
    )
    field_type = Column(
        SQLAlchemyEnum(AttributeFieldTypeEnum),
        nullable=False,
        default=AttributeFieldTypeEnum.TEXT,
    )
    description = Column(Text, nullable=True, comment="Ajuda sobre o atributo")
    default_value = Column(
        String, nullable=True
    )  # Valor padrão como string, conversão no frontend/lógica
    options = Column(
        JSON,
        nullable=True,
    ...
    __table_args__ = (
        UniqueConstraint(
            "product_type_id", "attribute_key", name="_product_type_attribute_key_uc"
        ),
    )
...
class Produto(Base):
    ...
    # Nomes e Descrições
    nome_base = Column(
        String(255), nullable=False, comment="Nome principal/original do produto"
    )
    nome_chat_api = Column(
        String(255), nullable=True, comment="Nome otimizado/gerado pela IA"
    )
    ...
    dimensoes_cm = Column(
        String(50), nullable=True, comment="Formato AxLxP, ex: 10x15x20"
    )

    # Relacionamentos
    fornecedor_id = Column(Integer, ForeignKey("fornecedores.id"), nullable=True)
    product_type_id = Column(
        Integer, ForeignKey("product_types.id"), nullable=True
    )  # Associa ao tipo de produto

    # Categorização e SEO
    categoria_original = Column(String(150), nullable=True)
    categoria_mapeada = Column(
        String(150), nullable=True
    )  # Categoria após algum processamento/padronização
    tags_palavras_chave = Column(
        MutableList.as_mutable(JSON), nullable=True
    )  # Lista de strings

    # Mídia
    imagem_principal_url = Column(String, nullable=True)  # URL da imagem principal
    imagens_secundarias_urls = Column(
        MutableList.as_mutable(JSON), nullable=True
    )  # Lista de URLs de imagens secundárias
    ...
    status_enriquecimento_web = Column(
        ...
        default=StatusEnriquecimentoEnum.NAO_INICIADO,
    )
    status_titulo_ia = Column(
        SQLAlchemyEnum(StatusGeracaoIAEnum), default=StatusGeracaoIAEnum.NAO_INICIADO
    )
    ...
    dynamic_attributes = Column(
        MutableDict.as_mutable(JSON),
        nullable=True,
        comment="Atributos dinâmicos preenchidos baseados no ProductType (JSON)",
    )

    # Histórico de mensagens do processo de enriquecimento web/IA.
    # Estrutura flexível armazenada como JSON (lista ou dict).
    log_enriquecimento_web = Column(
        JSON, nullable=True, comment="Log de enriquecimento web do produto"
    )
    ...
    log_processamento = Column(
        ...
        nullable=True,
        comment="Lista de mensagens/eventos de log para este produto",
    )
    ...
    registros_uso_ia = relationship(
        "RegistroUsoIA", back_populates="produto", cascade="all, delete-orphan"
    )
...
class RegistroUsoIA(Base):
    ...
    )  # Se o produto for deletado, manter o registro de uso

    tipo_acao = Column(
        SQLAlchemyEnum(TipoAcaoEnum, name="tipoacaoenum"), nullable=False
    )  # Usar o Enum Python
    provedor_ia = Column(String, nullable=True)  # ex: "openai", "gemini"
    modelo_ia = Column(
        String, nullable=True
    )  # ex: "gpt-3.5-turbo", "gemini-1.5-flash-latest"
    ...
    creditos_consumidos = Column(
        Integer, nullable=False, default=1
    )  # Quantidade de "operações" ou créditos específicos da plataforma
...
class RegistroHistorico(Base):
    """Encapsulates Registro historico."""
...
class CatalogImportFile(Base):
    """Encapsulates Catalog import file."""
...
class FornecedorImportJob(Base):
    """Encapsulates Fornecedor import job."""
