...
if settings.GOOGLE_CLIENT_ID and settings.GOOGLE_CLIENT_SECRET:
    ...
    logger.warning('Credenciais do Google OAuth (CLIENT_ID ou CLIENT_SECRET) nao configuradas no .env. Login com Google desabilitado.')
if settings.FACEBOOK_CLIENT_ID and settings.FACEBOOK_CLIENT_SECRET:
    if 'facebook' not in oauth._clients:
        oauth.register(name='facebook', client_id=settings.FACEBOOK_CLIENT_ID, client_secret=settings.FACEBOOK_CLIENT_SECRET, authorize_url='https://www.facebook.com/v19.0/dialog/oauth', access_token_url='https://graph.facebook.com/v19.0/oauth/access_token', userinfo_endpoint='https://graph.facebook.com/me?fields=id,name,email,first_name,last_name,picture', client_kwargs={'scope': 'email public_profile'})
else:
...
class _AuthWorkflow:
    ...
    async def login_for_access_token(self, form_data: OAuth2PasswordRequestForm, db: Session) -> Dict[str, str]:
        return await self._runtime.login_for_access_token(form_data=form_data, db=db)

    async def refresh_access_token(self, refresh_token_data: schemas.RefreshTokenRequest, db: Session) -> Dict[str, str]:
        return await self._runtime.refresh_access_token(refresh_token_data=refresh_token_data, db=db)

    async def update_users_me(self, user_update: schemas.UserUpdate, current_user: models.User, db: Session) -> models.User:
        return await self._runtime.update_users_me(user_update=user_update, current_user=current_user, db=db)

    async def change_password_me(self, payload: schemas.UserChangePassword, current_user: models.User, db: Session) -> Dict[str, str]:
        return await self._runtime.change_password_me(payload=payload, current_user=current_user, db=db)

    async def process_google_login(self, db: Session, google_userinfo: Dict[str, Any]) -> Optional[models.User]:
...
class _AuthRuntime:
    ...
    def create_access_token(self, data: dict, expires_delta: Optional[timedelta]=None) -> str:
        to_encode = data.copy()
        expire = datetime.now(timezone.utc) + expires_delta if expires_delta else datetime.now(timezone.utc) + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
        to_encode.update({'exp': expire, 'iat': datetime.now(timezone.utc)})
        return jwt.encode(to_encode, settings.SECRET_KEY, algorithm=settings.ALGORITHM)

    def create_refresh_token(self, data: dict, expires_delta: Optional[timedelta]=None) -> str:
        to_encode = data.copy()
        expire = datetime.now(timezone.utc) + expires_delta if expires_delta else datetime.now(timezone.utc) + timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS)
        to_encode.update({'exp': expire, 'iat': datetime.now(timezone.utc)})
    ...
    async def get_current_user(self, token: str, db: Session) -> models.User:
        credentials_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Nao foi possivel validar as credenciais', headers={'WWW-Authenticate': 'Bearer'})
        try:
    ...
    async def login_for_access_token(self, form_data: OAuth2PasswordRequestForm, db: Session) -> Dict[str, str]:
        ...
        access_token = self.create_access_token(data={'sub': user.email, 'user_id': user.id}, expires_delta=access_token_expires)
    ...
    async def refresh_access_token(self, refresh_token_data: schemas.RefreshTokenRequest, db: Session) -> Dict[str, str]:
        ...
        credentials_exception = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Refresh token invalido', headers={'WWW-Authenticate': 'Bearer'})
        try:
            ...
            if not user or user.email != email or (not user.is_active):
                raise credentials_exception
            access_token_expires = timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
            new_access_token = self.create_access_token(data={'sub': user.email, 'user_id': user.id}, expires_delta=access_token_expires)
    ...
    async def update_users_me(self, user_update: schemas.UserUpdate, current_user: models.User, db: Session) -> models.User:
        ...
        if 'password' in update_data:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Use o endpoint de alteracao de senha para atualizar a senha.')
        ...
        return self._users(db).update_user(db_user=current_user, user_update=user_update)

    async def change_password_me(self, payload: schemas.UserChangePassword, current_user: models.User, db: Session) -> Dict[str, str]:
        if not current_user.hashed_password or not self.verify_password(payload.current_password, current_user.hashed_password):
    ...
    async def _get_or_create_social_user(self, db: Session, email: str, nome: Optional[str], provider: str, provider_user_id: str) -> Optional[models.User]:
        ...
        user_in_create = schemas.UserCreateOAuth(email=email, nome_completo=nome or email.split('@')[0], provider=provider, provider_user_id=provider_user_id)
        created_user = self._users(db).create_user_oauth(user_oauth=user_in_create, plano_id_default=default_plano.id if default_plano else None)
    ...
    async def process_google_login(self, db: Session, google_userinfo: Dict[str, Any]) -> Optional[models.User]:
        ...
        if not google_user_id:
            ...
            return None
        return await self._get_or_create_social_user(db=db, email=email, nome=nome_completo, provider='Google', provider_user_id=google_user_id)

    async def process_facebook_login(self, db: Session, facebook_userinfo: Dict[str, Any]) -> Optional[models.User]:
        email = facebook_userinfo.get('email')
        if not email:
            logger.error('Email do Facebook nao fornecido. Nao e possivel prosseguir com login/registro baseado em email.')
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Email nao fornecido pelo Facebook. Verifique suas permissoes.')
...
class _AuthDependencies:
    ...
    async def get_current_user(token: str=Depends(oauth2_scheme), db: Session=Depends(get_db)) -> models.User:
        ...
        return await AuthWorkflow().get_current_user(token=token, db=db)

class _AuthActiveUserDependency:

    @staticmethod
    async def get_current_active_user(current_user: models.User=Depends(_AuthDependencies.get_current_user)) -> models.User:
...
class _EndpointHandlers:
    ...
    async def login_for_access_token(form_data: OAuth2PasswordRequestForm=Depends(), db: Session=Depends(get_db)):
        ...
        return await AuthWorkflow().login_for_access_token(form_data=form_data, db=db)

    @router.post('/token/refresh/', response_model=schemas.Token)
    async def refresh_access_token(refresh_token_data: schemas.RefreshTokenRequest, db: Session=Depends(get_db)):
        """Endpoint de refresh token."""
        return await AuthWorkflow().refresh_access_token(refresh_token_data=refresh_token_data, db=db)

    @router.get('/users/me', response_model=schemas.UserResponse)
    async def read_users_me(current_user: models.User=Depends(_AuthActiveUserDependency.get_current_active_user)):
        """Retorna perfil do usuario autenticado."""
        return current_user

    @router.put('/users/me', response_model=schemas.UserResponse)
    async def update_users_me(user_update: schemas.UserUpdate, current_user: models.User=Depends(_AuthActiveUserDependency.get_current_active_user), db: Session=Depends(get_db)):
        """Atualiza dados de perfil do usuario autenticado."""
        return await AuthWorkflow().update_users_me(user_update=user_update, current_user=current_user, db=db)

    @router.put('/users/me/change-password')
    async def change_password_me(payload: schemas.UserChangePassword, current_user: models.User=Depends(_AuthActiveUserDependency.get_current_active_user), db: Session=Depends(get_db)):
        """Atualiza senha do usuario autenticado."""
        return await AuthWorkflow().change_password_me(payload=payload, current_user=current_user, db=db)
