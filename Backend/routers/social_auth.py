...
class _ModuleAliasProviders:
    ...
    @staticmethod
    def get_social_auth_router_workflow():
...
class _SocialAuthRouterRuntime:
    ...
    async def process_google_login(self, db: Session, userinfo):
        return await self._auth_workflow.process_google_login(db=db, google_userinfo=userinfo)

    async def process_facebook_login(self, db: Session, userinfo):
        return await self._auth_workflow.process_facebook_login(db=db, facebook_userinfo=userinfo)
...
class _SocialAuthRouterWorkflow:
    ...
    def social_login_config(self) -> schemas.SocialLoginConfig:
        return schemas.SocialLoginConfig(google_enabled=self._runtime.has_client('google'), facebook_enabled=self._runtime.has_client('facebook'))

    async def google_login(self, request: Request):
        if not self._runtime.has_client('google'):
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail='Google OAuth nao configurado.')
    ...
    async def google_callback(self, request: Request, db: Session) -> schemas.Token:
        ...
        if not user:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Nao foi possivel autenticar o usuario Google.')
    ...
    async def facebook_login(self, request: Request):
        if not self._runtime.has_client('facebook'):
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail='Facebook OAuth nao configurado.')
    ...
    async def facebook_callback(self, request: Request, db: Session) -> schemas.Token:
        ...
        if not user:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Nao foi possivel autenticar o usuario Facebook.')
        access = self._runtime.create_access_token({'sub': user.email, 'user_id': user.id})
        refresh = self._runtime.create_refresh_token({'sub': user.email, 'user_id': user.id})
        return schemas.Token(access_token=access, refresh_token=refresh, token_type='bearer')
...
class _SocialAuthRequestScope:
    ...
    def __init__(self, db: Session, workflow: SocialAuthRouterWorkflow | None=None) -> None:
        self._db = db
        self._workflow = workflow or _ModuleAliasProviders.get_social_auth_router_workflow()

    async def google_callback(self, request: Request) -> schemas.Token:
        return await self._workflow.google_callback(request=request, db=self._db)

    async def facebook_callback(self, request: Request) -> schemas.Token:
        return await self._workflow.facebook_callback(request=request, db=self._db)
_build_social_auth_request_workflow = ServiceContainerDependencySupport.build_request_scoped_dependency(lambda session: _SocialAuthRequestScope(db=session))
...
class _EndpointHandlers:
    ...
    async def social_login_config():
        ...
        workflow = _ModuleAliasProviders.get_social_auth_router_workflow()
    ...
    async def google_callback(request: Request, request_workflow: _SocialAuthRequestScope=Depends(_build_social_auth_request_workflow)):
    ...
    async def facebook_callback(request: Request, request_workflow: _SocialAuthRequestScope=Depends(_build_social_auth_request_workflow)):
