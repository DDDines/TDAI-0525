...
class _AuthUtilsWorkflow:
    ...
    async def get_current_user(self, request: Request, db: Session, token: str) -> models.User:
        _ = request
...
_build_auth_request_session = ServiceContainerDependencySupport.build_request_scoped_dependency(lambda session: session)

class _AuthUtilsCurrentUserDependency:

    @staticmethod
    async def get_current_user(request: Request, session: Session=Depends(_build_auth_request_session), token: str=Depends(oauth2_scheme)) -> models.User:
        """Resolve usuario autenticado atual via workflow OO."""
        workflow = _ModuleAliasProviders.get_auth_utils_workflow()
        return await workflow.get_current_user(request=request, db=session, token=token)

class _AuthUtilsActiveUserDependency:

    @staticmethod
    async def get_current_active_user(current_user: models.User=Depends(_AuthUtilsCurrentUserDependency.get_current_user)) -> models.User:
...
class _AuthUtilsSuperUserDependency:
    ...
    @staticmethod
    async def get_current_active_superuser(current_user: models.User=Depends(_AuthUtilsActiveUserDependency.get_current_active_user)) -> models.User:
