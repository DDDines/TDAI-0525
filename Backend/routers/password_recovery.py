...
class _ModuleAliasProviders:
    ...
    @staticmethod
    def get_password_recovery_workflow():
...
class _PasswordRecoveryRuntime:
    ...
    def set_user_password_reset_token(self, db: Session, user, *, token_hash: str, expires_at: datetime) -> None:
        UserRepository(db).set_user_password_reset_token(user=user, token_hash=token_hash, expires_at=expires_at)

    async def send_password_reset_email(self, *, email_to: str, username: str, reset_link: str) -> None:
        await self._email_workflow.send_password_reset_email(email_to=email_to, username=username, reset_link=reset_link)

    def get_user_by_reset_token(self, db: Session, token_hash: str):
        return UserRepository(db).get_user_by_reset_token(token_hash=token_hash)
...
class _PasswordRecoveryWorkflow:
    ...
    async def recover_password(self, db: Session, email: str, request: Request) -> schemas.Msg:
        _ = request
        user = self._runtime.get_user_by_email(db, email=email)
        if not user:
            return schemas.Msg(msg='Se um usuario com este email existir, um link de recuperacao foi enviado.')
        ...
        self._runtime.set_user_password_reset_token(db, user, token_hash=token_hash, expires_at=expires_at)
        reset_link = f'{settings.FRONTEND_URL}/reset-password?token={token}'
        try:
            await self._runtime.send_password_reset_email(email_to=user.email, username=user.nome_completo or user.email, reset_link=reset_link)
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Houve um erro ao enviar o email de recuperacao. Tente novamente mais tarde.') from exc

    def reset_password(self, db: Session, reset_data: schemas.PasswordResetSchema) -> schemas.Msg:
        ...
        if not user:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Token de reset invalido.')
...
class _PasswordRecoveryRequestScope:
    ...
    def __init__(self, db: Session, workflow: PasswordRecoveryWorkflow | None=None) -> None:
        self._db = db
        self._workflow = workflow or _ModuleAliasProviders.get_password_recovery_workflow()

    async def recover_password(self, *, email: str, request: Request) -> schemas.Msg:
        return await self._workflow.recover_password(db=self._db, email=email, request=request)

    def reset_password(self, *, reset_data: schemas.PasswordResetSchema) -> schemas.Msg:
        return self._workflow.reset_password(db=self._db, reset_data=reset_data)
_build_password_recovery_request_workflow = ServiceContainerDependencySupport.build_request_scoped_dependency(lambda session: _PasswordRecoveryRequestScope(db=session))

class _EndpointHandlers:

    @router.post('/password-recovery/{email}', response_model=schemas.Msg)
    async def recover_password(email: str, request: Request, request_workflow: _PasswordRecoveryRequestScope=Depends(_build_password_recovery_request_workflow)):
        """Endpoint HTTP que delega a execucao para workflow/servico OO (recover_password)."""
        return await request_workflow.recover_password(email=email, request=request)

    @router.post('/reset-password/', response_model=schemas.Msg)
    def reset_password(*, reset_data: schemas.PasswordResetSchema=Body(...), request_workflow: _PasswordRecoveryRequestScope=Depends(_build_password_recovery_request_workflow)):
        """Endpoint HTTP que delega a execucao para workflow/servico OO (reset_password)."""
        return request_workflow.reset_password(reset_data=reset_data)
