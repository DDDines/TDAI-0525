...
router = APIRouter(prefix='/search', tags=['Search'], dependencies=[Depends(auth_utils._AuthUtilsActiveUserDependency.get_current_active_user)])

class _SearchRuntime:
    """Runtime OO para consulta global de entidades no search."""

    def search_all(self, db: Session, current_user: models.User, q: Optional[str], limit: int) -> schemas.SearchResults:
        ...
        for product_type in pt_query.order_by(models.ProductType.created_at.desc()).limit(limit).all():
            results_items.append((product_type.created_at, schemas.SearchItem(id=product_type.id, type='tipo_produto', name=product_type.friendly_name)))
...
class _SearchWorkflow:
    ...
    def search_all(self, db: Session, current_user: models.User, q: Optional[str], limit: int) -> schemas.SearchResults:
        return self._runtime.search_all(db=db, current_user=current_user, q=q, limit=limit)
...
class _SearchRequestScope:
    ...
    def __init__(self, db: Session, workflow: SearchWorkflow | None=None) -> None:
        self._db = db
        self._workflow = workflow or _ModuleAliasProviders.get_search_workflow()

    def search_all(self, *, current_user: models.User, q: Optional[str], limit: int) -> schemas.SearchResults:
        return self._workflow.search_all(db=self._db, current_user=current_user, q=q, limit=limit)
_build_search_request_workflow = ServiceContainerDependencySupport.build_request_scoped_dependency(lambda session: _SearchRequestScope(db=session))

class _EndpointHandlers:

    @router.get('/', response_model=schemas.SearchResults)
    def search_all(q: Optional[str]=Query(None, min_length=1), limit: int=Query(10, ge=1, le=50), request_workflow: _SearchRequestScope=Depends(_build_search_request_workflow), current_user: models.User=Depends(auth_utils._AuthUtilsActiveUserDependency.get_current_active_user)):
        """Endpoint HTTP que delega a execucao para workflow/servico OO (search_all)."""
        return request_workflow.search_all(current_user=current_user, q=q, limit=limit)
