...
class _InitialDataRuntime:
    ...
    def _ensure_default_roles(*, user_repo: UserRepository) -> Tuple[Optional[Role], Optional[Role]]:
        roles_padrao = [
            ...
            {"name": "user", "description": "Usuario padrao da plataforma"},
        ]
    ...
    def _ensure_default_plans(*, user_repo: UserRepository) -> None:
        planos_padrao = [
            {
                "nome": "Gratuito",
                "descricao": "Plano basico com limitacoes.",
                "preco_mensal": 0.0,
    ...
    def _ensure_admin_user(self, *, db: Session, user_repo: UserRepository) -> Optional[User]:
        ...
        db_admin_user = user_repo.create_user(
    ...
    def _ensure_global_product_types(*, product_type_repo: ProductTypeRepository) -> None:
        tipos_produto_globais = [
            {
                ...
                "attribute_templates": [
                    ...
                    schemas.AttributeTemplateCreate(
                        attribute_key="cor_predominante",
                    ...
                    schemas.AttributeTemplateCreate(
                        attribute_key="garantia_meses",
                        label="Garantia (meses)",
                        field_type=AttributeFieldTypeEnum.NUMBER,
                        default_value="12",
                        display_order=3,
                    ),
                ],
        ...
        for pt_data in tipos_produto_globais:
            ...
            try:
                ...
                logger.warning(
                    "Nao foi possivel criar o tipo de produto global '%s': %s",
                    pt_create_schema.key_name,
                    exc,
                )
            except Exception as exc:
                logger.error(
                    "Erro inesperado ao criar tipo de produto global '%s': %s",
                    pt_create_schema.key_name,
                    exc,
                    exc_info=True,
                )
