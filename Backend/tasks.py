...
class _TaskRuntime:
    ...
    def _resolve_catalog_file_path(self, stored_filename: str) -> Path:
        ...
        if file_path.is_absolute():
            return file_path
        return Path(__file__).resolve().parent / 'static' / 'uploads' / 'catalogs' / stored_filename
    ...
    def process_pdf_extraction_task(self, import_job_id: int, page_number: int, db_url: str) -> None:
        ...
        try:
            ...
            result = self._file_processing_service.extract_data_from_single_page(str(file_path), page_number)
