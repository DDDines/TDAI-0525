...
import enum
from pydantic import HttpUrl # Adicionado
...
def get_fornecedores_by_user(
    ...
    skip: int = 0, 
    limit: int = 100,
    termo_busca: Optional[str] = None # Adicionado termo_busca para consistência com contagem
) -> List[models.Fornecedor]:
    query = db.query(models.Fornecedor).filter(models.Fornecedor.user_id == user_id)
    if termo_busca: # Adicionar filtro de busca se fornecido
...
# NOVO: Função para contar fornecedores com base nos filtros
def count_fornecedores_by_user(
    db: Session,
    user_id: int,
    termo_busca: Optional[str] = None # Adicionado termo_busca
) -> int:
...
def update_fornecedor(db: Session, db_fornecedor: models.Fornecedor, fornecedor_update: schemas.FornecedorUpdate) -> models.Fornecedor:
    ...
    if "site_url" in update_data:
        ...
        # Se não for HttpUrl nem None, e 'exclude_unset=True' não removeu, pode causar erro.
        # Mas o Pydantic já deve ter validado ou convertido para HttpUrl ou None.
        if "site_url" in update_data: del update_data["site_url"] # Remove para não ser processado no loop abaixo
...
# --- Produto CRUD ---
# ... (O resto do CRUD de Produto, UsoIA, etc., permanece o mesmo que na sua última versão funcional)
