...
async function safeStep(name, fn) {
  ...
  try {
    ...
    if (failed) {
      try {
        await dismissBlockingOverlays(globalThis.__auditPageRef);
      } catch {
...
async function closeModalsIfAny(page) {
  ...
  if (await closeByAria.first().isVisible().catch(() => false)) {
    await closeByAria.first().click({ force: true }).catch(() => {});
  }
...
async function dismissBlockingOverlays(page) {
  ...
  if (await overlay.count()) {
    const closeBtn = overlay.locator('.modal-close-button, .modal-close, button[aria-label="Fechar"]').first();
    if (await closeBtn.isVisible().catch(() => false)) {
      await closeBtn.click({ force: true }).catch(() => {});
    } else {
      await page.keyboard.press('Escape').catch(() => {});
      await overlay.first().evaluate((el) => {
        const btn = el.querySelector('.modal-close-button, .modal-close, button[aria-label="Fechar"]');
...
async function runUiAudit() {
  ...
  globalThis.__auditPageRef = page;
  page.setDefaultTimeout(15000);

  page.on('console', (msg) => {
    if (msg.type() === 'error') {
      consoleErrors.push(`[${activeStep}] ${msg.text()}`);
    }
  });
  ...
  await safeStep('Fornecedor info: salvar alterações', async () => {
    await page.goto(`${BASE_URL}/fornecedores`, { waitUntil: 'domcontentloaded' });
    await clickIfVisible(page.locator('#forn-table tbody tr').first());
    await page.getByRole('heading', { name: /editar fornecedor/i }).waitFor({ state: 'visible' });
  ...
  await safeStep('Fornecedor importação: abrir wizard', async () => {
    ...
    await clickIfVisible(page.locator('#forn-table tbody tr').first());
    await page.getByRole('heading', { name: /editar fornecedor/i }).waitFor({ state: 'visible' });
    await page.locator('#edit-forn-modal .tab-navigation button', { hasText: /importar cat/i }).first().click();
    const launchBtn = page.locator('#edit-forn-modal .form-section button', { hasText: /importar cat/i }).first();
    await launchBtn.waitFor({ state: 'visible' });
    await launchBtn.click();
    await page.locator('#wizard-file-input').waitFor({ state: 'visible', timeout: 15000 });
  });
